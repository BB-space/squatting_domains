...
def analyze_compressed_domains(args):
    ...
    squat_dict_multiple_domain = args['squat_dict']
    ...
    for line in f.readlines():
        ...
        if len(domain_array) > 1:
            ...
            try:
                ...
                for original_domain_tld in squat_dict_multiple_domain:

                    squat_dict = squat_dict_multiple_domain[original_domain_tld]
...
import copy
def recursively_analyze_gz_files(direcory, original_domain_tlds, output_dir=None):

    squat_dict_multiple_domains = {}

    for original_domain_tld in original_domain_tlds:
        ...
        squat_dict_multiple_domains[original_domain_tld] = copy.deepcopy(squat_dict)
        del squat_dict
    ...
    for i in files:
        args = dict()
        args['gz_file'] = i
        args['squat_dict'] = squat_dict_multiple_domains
        args['original_domain'] = None
    ...
    if not output_dir:
        f_name = directory.replace(u'/', '_') + 'all' + '.out'
    else:
        f_name = directory.replace(u'/', '_') + 'all' + '.out'
...
if __name__ == "__main__":
    ...
    #domains = domains[20:40]


    original_domain_tlds = list()
    for i in domains:
        original_domain_tld = i.decode('utf-8')
        original_domain_tlds.append(original_domain_tld)

    recursively_analyze_gz_files(direcory=directory, original_domain_tlds=original_domain_tlds, output_dir=None)
